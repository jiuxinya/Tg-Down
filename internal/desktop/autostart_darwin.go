...
const darwinPlistName = "app.tg-down.desktop.plist"
...
` + ""
	...
	_ = exec.Command("launchctl", "load", "-w", p).Run() //nolint:gosec // 固定路径与参数
	return nil
}
...
func (darwinAutostart) Disable() error {
	...
	_ = exec.Command("launchctl", "unload", "-w", p).Run() //nolint:gosec // 同上
	err = os.Remove(p)
	if os.IsNotExist(err) {
		return nil
	}
