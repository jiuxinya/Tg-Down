...
> 状态：v3.0 已于 2026-08-22 发布，勾选状态按 2026-08-24 的逐项核对回填。
>
> - `[x]` = v3.0 已交付
> - `[~]` = v3.0 未交付，已在 v3.2 补齐（见 [v3.2-plan.md](v3.2-plan.md)）
> - `[ ]` = 仍未做（判断为收益不足或推后，理由见 v3.2-plan.md）
...
- [x] **单文件失败不影响任务终态**（`telegram/client.go:1180`、`queue/manager.go:407`）：
      下载错误在 dispatch goroutine 里被吞掉 → 全部文件失败的任务仍标记 completed。
      新增 `partial_failed` 终态，任务卡片显示"完成 N / 失败 M"
- [x] **FLOOD_WAIT 重试**（`retry/retry.go:62-89`）：现用错误字符串子串匹配，429 / FLOOD_WAIT
...
- [x] **替换 8 空页启发式**（`client.go:54-63`）：连续 8 个空页判定历史结束，容忍窗口仅 ~25s；
      冷缓存时静默误判扫完并标记成功（漏下载且无告警）
- [x] **去重路径移到信号量之后**（`downloader.go:770-844`）：去重 SQL 查询与 `copyFromDuplicate`
...
- [x] **真失败的文件纳入补下**（`store/history.go:106-109`）：恢复只捞 `reason='interrupted'`，
      网络/磁盘错误失败的行永远不会被补下
- [x] **跳过判断校验大小**（`downloader.go:794-798`）：`os.Stat` 命中即跳过，0 字节残留文件被永久跳过
- [x] `downloading` 状态虚高（`downloader.go:750` vs `:844`）：`RecordStarted` 在 acquire 前发出，
      DB 里可能 100 行 downloading 而实际只下 5 个
- [x] 磁盘满快速失败（`client.go:914-929`）：现对任何 rename 失败无脑重试 5×500ms + 全量 copy
- [x] goroutine 泄漏（`client.go:51,259-264`）：`tdCall` 的 24h fallback 让卡住的下载泄漏 goroutine 一天
...
- [x] 连接状态进 `/api/state`（`client.go:1514` 的 `UpdateConnectionState` 只打日志，
      UI 无法显示"正在等待网络"）
- [x] **telegram 包注入缝**（与 M6 联动）：`Client.td` 是具体类型 `*tdclient.Client`，
...
- [x] **服务端枚举**：`SearchChatMessages` + `SearchMessagesFilter` 替代全量翻页
      （`client.go:978-985` 已有 filter 映射，只用在 `GetChatMessageCount`）
- [x] **定时任务增量扫描**（`manager.go:409`）：任务完成后清零 `scan_cursor` →
      每次定时触发都从最新消息重扫整条历史。注释里的"去重使重扫廉价"只对下载成立，对扫描不成立
- [x] `DateTo` skip-ahead（`client.go:1393-1408`）：现在只能从最新一路翻过来逐条丢弃
- [x] DB 索引：补 `(chat_id, created_at)` / `(status, created_at)` / `(media_type, created_at)` 复合索引；
...
- [x] `synchronous=NORMAL`；每媒体 2 次独立事务（`history.go:23` + `:66`）改批量提交
- [x] `schema_version` 表（`store.go:31` 无版本表，两条 O(N) 归一化 UPDATE 每次启动无条件跑）

## M3 下载能力深化

- [x] **统一媒体类型词汇源**：`downloader.go:32-38` 与 `client.go:68-73` 各定义一套常量，
...
- [x] 新增 sticker / video note。注意 TDLib **没有 sticker 的 SearchMessagesFilter**，
      其服务端计数架构上不可能准确，需单独处理；`Sticker` 结构无 `FileName`，需合成文件名
- [x] **文件名/目录模板**：`{chat_title}` `{date}` `{sender}` `{album}` `{msg_id}` `{ext}` 占位符
      + 同名冲突策略。今天全硬编码（`client.go:770,800,849`、`downloader.go:945-959`），
      chat 目录只用数字 ID（`chat_-1001234567890/`），尽管 `chat_title` 已落库。
      **默认模板 = 现有布局**，避免旧文件重下
- [x] 关键词 / 发送者过滤（`MediaInfo.SenderID` 已提取但未使用）
- [x] 聊天导出归档（HTML / JSON，类似 Telegram Desktop export）

## M4 界面重构 + 媒体预览画廊

画廊后端是零起点：`ServeFile` / `FileServer` / `thumbnail` 全库零命中，**TDLib 层从未请求过缩略图**。

- [x] TDLib 缩略图 / minithumbnail 获取
- [x] 媒体服务端点。两个必须先堵的泄漏点：`.tdlib-files`（`client.go:155`，就在下载根目录内）
      ...
      的判定逻辑可复用到读取侧
- [x] `album_id` 加进 API DTO（`store.go:74` 已落库、文件已按 `album_<id>` 分目录、索引已建，
      唯独 `handlers.go:611-626` 的 DTO 把它丢了）→ 画廊按相册分组
- [x] **前端 Vite 重构**：1638 行单文件已有 4 组重复 DOM 代码（媒体卡片渲染两遍、聊天下拉三遍、
      并发输入框两处、全部暂停按钮两处）；30 处 `innerHTML` 全量重建 + 1Hz state 推送 →
      图片节点每秒销毁重建。产物仍 `go:embed`
- [x] 增量渲染替代全量 `innerHTML`；SSE 的 `task` 事件 payload 目前被完全丢弃，
      只触发 `/api/tasks` 全量重拉（`index.html:1624`）
- [x] CSP / X-Frame-Options / X-Content-Type-Options（今天一个安全响应头都没有，
...
- [x] 静态库列表提取单一来源（现在 `Makefile:12` / `.github/actions/setup-tdlib/action.yml:26` /
      `Dockerfile:27` / `release.yml:60` 四处各写一遍）
- [x] `scripts/install-tdlib.sh:24` 的 `case "$(uname -s)"` 无 Windows 分支且无 default →
      MSYS2 下静默跳过依赖安装。补 MinGW-w64 分支（CGo 只能用 gcc 系，MSVC 路线不可行）
- [x] 链接 flag：`-Wl,-rpath`（PE 无此概念）、`-ldl`（Windows 无 libdl）需 Windows 变体
- [x] `cmd/main.go:182` 的 `syscall.SIGTERM` 在 Windows Go 里不存在 → 编译失败
- [x] `sanitizeFileName`（`downloader.go:966-985`）不处理 CON/PRN/NUL 保留名、长度上限、首尾空格点
- [x] CI 加 windows job；发布产物 `.zip` + `tg-down.exe`（`release.yml:73-88` 的
...
- [x] telegram / web 包接口缝（项目已有此习惯：`queue.ChatDownloader`（`queue/queue.go:43`）与
...
- [x] `make test` + 覆盖率（Makefile 无 test target；CI 是裸 `go test -v ./...`，无 `-race` 无 `-cover`）
